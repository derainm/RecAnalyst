    ...
    public function run()
    {
        ...
        if ($this->showPlayerUnits) {
            foreach ($header->playerInfo->playerObjects as $object) {
                ...
                //we get coordinate and check if it is corrupted or not
                //$x1 = NAN mean play postion has bug so we get tc player position
                list ($x1, $y1) =$object->owner->initialState->position; 
                if(is_nan($x1) || is_nan($y1))
                {  
                    //todo if nomade get villager instead of villagergers?
                    if( $object->id ==109 )//
                    { 
                        $object->owner->initialState->position = $object->position;;
                    }
                }
                //echo "id:". $object->id;
        ...
        if ($this->showPositions) {
            foreach ($header->players as $player) {
                ...
                //if $x = NAN or $y = NAN  then we don't draw 
                // $x = NAN or $y = NAN  happen whenserializer is broken and radnom value break mpa generation
